<template>
  <div class="article-container">
    ...
    <div class="main-wrap">
      ...
      <div v-else-if="article.title" class="article-detail">
        ...
        <van-cell class="user-info" center :border="false">
          ...
          <div slot="label" class="publish-date">{{ article.pubdate | ralativeTime }}</div>
          <!-- 模板中的$event是事件参数
          ...
          一个组件上只能用一次 v-model
          如果有多个数据需要实现类似于 v-model的效果，咋办？
          可以使用属性的 .sync 修饰符
          -->
          <follow-user class="follow-btn" :user-id="article.aut_id" v-model="article.is_followed" />
          <!-- <van-button class="follow-btn" round size="small" v-if="article.is_followed" :loading="followLoading"
            @click="onFollow">已关注
          </van-button>
          <van-button class="follow-btn" type="info" color="#3296fa" round size="small" icon="plus"
            :loading="followLoading" @click="onFollow" v-else>关注
          </van-button> -->
        </van-cell>
        <!-- /用户信息 -->
        ...
        <!-- 文章评论列表 -->
        <ArticleComment :source="article.art_id" @onload-success="totalCommentCount = $event.total_count"
        ...
        <div class="article-bottom">
          ...
          <!-- 收藏 -->
          <CollectArticle class="van-icon" v-model="article.is_collected" :article-id="article.art_id" />
          <!-- /收藏 -->
    ...
    <!-- 评论回复 -->
    <!-- 弹出层是懒渲染的，只要在第一次展示的时候才会渲染里面的内容，之后它的关闭和显示都是在切换内容的显示和隐藏 -->
